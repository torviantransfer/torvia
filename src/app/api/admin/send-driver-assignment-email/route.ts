    ...
    const { data: assignment, error } = await supabase
      ...
      .select(
        `id, leg, pickup_time, status,
         ...
         reservations(reservation_code, direction, pickup_datetime, return_datetime, locale, regions(name_en, name_tr, name_de, name_pl, name_ru, name_nl, name_ro), customers(first_name, last_name, email))`
      )
      .eq("id", assignmentId)
      .single();
