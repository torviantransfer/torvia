...
import {
  ...
  BarChart2, ExternalLink, ChevronRight, MousePointerClick, ShoppingCart,
...
/**
 * 30-day historical view of visitor traffic — lives as the "Genel Analitik"
 * tab next to the real-time LiveVisitors tab. Was previously bolted onto the
 * main Kontrol Paneli dashboard, crowding out the business (revenue/booking)
 * charts that actually belong there.
 */
export default function VisitorAnalyticsHistory() {
  ...
  if (!data) {
    return <p className="text-sm text-slate-400 text-center py-16">Veriler yüklenemedi</p>;
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        <SummaryCard icon={Activity} label="Son 30 Gün" value={summary.eventsLast30Days} sub="toplam olay" gradient="from-slate-500 to-slate-700" />
        <SummaryCard icon={Globe} label="Benzersiz Oturumlar" value={summary.uniqueSessions} sub="ziyaretçi" gradient="from-cyan-500 to-blue-600" />
        <SummaryCard icon={TrendingUp} label="Dönüşüm" value={`${summary.conversionRate}%`} sub="rezervasyon oranı" gradient="from-emerald-500 to-teal-600" />
        <SummaryCard icon={CheckCircle2} label="Tamamlanan Ödemeler" value={summary.paymentSuccessSessions} sub="başarılı ödeme" gradient="from-violet-500 to-purple-600" />
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 p-6" style={{ boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
        <div className="flex items-center gap-2 mb-6">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400">Ziyaretçiden ödemeye dönüşüm adımları · son 30 gün</p>
          </div>
        </div>
        <BookingFunnelSteps funnel={data.funnel} />
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-2xl border border-slate-100 p-5" style={{ boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
          ...
          <SourceList sources={data.topSources} />
        </div>
      </div>
...
function CountryList({ countries }: { countries: { name: string; value: number }[] }) {
  ...
  return (
    <div className="space-y-2.5">
      ...
      {countries.length === 0 && <p className="text-sm text-slate-400 text-center py-6">Henüz veri yok</p>}
    </div>
  );
}
...
function PageList({ pages }: { pages: { name: string; value: number }[] }) {
  ...
  return (
    <div className="space-y-2.5">
      ...
      {pages.length === 0 && <p className="text-sm text-slate-400 text-center py-6">Henüz veri yok</p>}
    </div>
  );
}
...
function SourceList({ sources }: { sources: { name: string; value: number }[] }) {
  ...
  return (
    <div className="space-y-2.5">
      ...
      {sources.length === 0 && <p className="text-sm text-slate-400 text-center py-6">Henüz veri yok</p>}
    </div>
  );
}
