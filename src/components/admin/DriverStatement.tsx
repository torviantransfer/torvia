...
const HERO = {
  owe: "bg-adm-amber-soft ring-adm-amber-line text-[#5c3a05]",
  owed: "bg-adm-rose-soft ring-[#f6c9d1] text-[#6e1524]",
  closed: "bg-adm-green-soft ring-[#bfe3cb] text-[#0d4d26]",
};

const TYPE_BADGE = {
  earning: "bg-adm-line-2 text-adm-ink-2",
  payment: "bg-adm-green-soft text-adm-green",
  adjustment: "bg-adm-amber-soft text-adm-amber",
};
    ...
    <div className="space-y-6">
      ...
      <div>
        <Link
          ...
          className="inline-flex items-center gap-1.5 text-xs font-semibold text-adm-muted hover:text-adm-ink"
        >
          <ArrowLeft size={13} />
          Şoför ödemeleri
        </Link>
        <div className="mt-3 flex flex-wrap items-center justify-between gap-4">
          <div className="flex min-w-0 items-center gap-4">
            ...
            <div className="min-w-0">
              <h1 className="flex flex-wrap items-center gap-2 text-2xl font-bold tracking-tight text-adm-ink">
                {s.driver.name}
                {!s.driver.active && (
                  <span className="rounded-md bg-adm-line-2 px-1.5 py-0.5 text-[11px] font-semibold text-adm-muted">pasif</span>
                )}
              </h1>
              <p className="mt-0.5 flex flex-wrap items-center gap-x-3 text-sm text-adm-muted">
                <span>Cari ekstre</span>
                {s.driver.phone && (
                  <a href={`tel:${s.driver.phone.replace(/\s/g, "")}`} className="inline-flex items-center gap-1 hover:text-adm-ink">
                    <Phone size={13} />
                    {s.driver.phone}
                  </a>
                )}
      ...
      {/* ── Today's balance ── */}
      <div className={`flex flex-wrap items-end justify-between gap-4 rounded-adm-lg px-6 py-5 ring-1 ${HERO[status.tone]}`}>
        <div>
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-px overflow-hidden rounded-adm-lg bg-adm-seg ring-1 ring-adm-line sm:grid-cols-5">
      ...
      {/* ── Tabs ── */}
      <section className="overflow-hidden rounded-adm-lg border border-adm-line bg-adm-surface">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-adm-line-2 px-3 pt-2">
            ...
            {(
              ...
              <button
                ...
                className={`-mb-px border-b-2 px-3 py-2.5 text-sm font-semibold transition ${
                  tab === key ? "border-adm-ink text-adm-ink" : "border-transparent text-adm-muted hover:text-adm-ink-2"
                }`}
              >
                {label} <span className="ms-1 rounded-full bg-adm-line-2 px-1.5 py-0.5 text-[11px] text-adm-muted">{count}</span>
              </button>
            ))}
          </div>
          {tab === "jobs" ? (
            <p className="px-2 pb-2 text-xs text-adm-muted">
              Müşteriden {fmtMoney(s.jobTotals.fareEur, "EUR")} · şoförlere {fmtMoney(s.jobTotals.feesEur, "EUR")} ·{" "}
              <span className="font-semibold text-adm-ink">bize kalan {fmtMoney(s.jobTotals.marginEur, "EUR")}</span>
            </p>
          ) : (
            <p className="px-2 pb-2 text-xs text-adm-muted">Hesap dolar; € ve ₺ kendi günlerinin kuruyla çevrilir</p>
          )}
        </div>
...
function Figure({ label, value, strong }: { label: string; value: string; strong?: boolean }) {
  return (
    <div className="bg-adm-surface px-4 py-3">
      ...
      <p className={`mt-1 text-lg ${strong ? "font-bold text-adm-ink" : "font-semibold text-adm-ink-2"}`}>{value}</p>
    </div>
  );
}

function LegFee({ cell }: { cell: LegCell | null }) {
  if (!cell) return <span className="text-adm-faint">—</span>;
  return (
    <span className={cell.mine ? "text-adm-ink" : "text-adm-muted"}>
      {cell.fee === null ? (
        <span className="text-xs font-semibold text-adm-amber">ücret yok</span>
      ) : (
...
function Margin({ job: j }: { job: JobRow }) {
  if (j.marginEur === null) return <span className="text-xs font-semibold text-adm-amber">eksik</span>;
  return (
    <span className={`font-bold tabular-nums ${j.marginEur < 0 ? "text-adm-rose" : "text-adm-ink"}`}>
...
function Jobs({ statement: s, adminBase }: { statement: Statement; adminBase: string }) {
  if (s.jobs.length === 0) return <p className="px-5 py-12 text-center text-sm text-adm-muted">Bu dönemde iş yok.</p>;
  const reservationHref = (code: string) => `${adminBase}/reservations/${encodeURIComponent(code)}`;

  return (
    <>
      {/* Phones: a card per job. */}
      <ul className="divide-y divide-adm-line-2 md:hidden">
        {s.jobs.map((j) => (
          <li key={j.reservationId} className="space-y-2 px-4 py-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                <Link href={reservationHref(j.code)} className="font-mono text-sm font-bold text-adm-ink">
                  {j.code}
                </Link>
            ...
            <p className="text-sm text-adm-ink-2">{j.route}</p>
            <p className="text-xs text-adm-muted">
              {[j.hotel, j.flightOut && `Uçuş ${j.flightOut}`, j.flightReturn && `Dönüş ${j.flightReturn}`, plates(j).join(" / ")]
                .filter(Boolean)
                .join(" · ")}
            </p>
            <dl className="grid grid-cols-3 gap-2 rounded-adm bg-adm-surface-2 p-2.5 text-xs">
              <div>
                <dt className="text-adm-muted">Müşteriden</dt>
                <dd className="font-semibold text-adm-ink">{fmtMoney(j.fare.value, j.fare.currency)}</dd>
              </div>
              <div>
                <dt className="text-adm-muted">Gidiş</dt>
                <dd className="font-semibold"><LegFee cell={j.outbound} /></dd>
              </div>
              <div>
                <dt className="text-adm-muted">Dönüş</dt>
      ...
      <div className="hidden overflow-x-auto md:block">
        <table className="w-full min-w-[1100px] text-sm">
          ...
          <tbody>
            {s.jobs.map((j) => (
              <tr key={j.reservationId} className="border-b border-adm-line-2 align-top last:border-0 hover:bg-adm-surface-2/60">
                <td className="whitespace-nowrap px-5 py-3 text-xs text-adm-ink-2">
                  {fmtDay(j.day)}
                  <div className="text-adm-muted">{j.time}</div>
                </td>
                <td className="px-3 py-3">
                  <Link href={reservationHref(j.code)} className="font-mono text-xs font-semibold text-adm-ink-2 hover:text-adm-brand-ink">
                    {j.code}
                  </Link>
                </td>
                <td className="px-3 py-3 text-xs">
                  <span className="font-semibold text-adm-ink">{TRIP_LABEL(j.tripType)}</span>
                  <div className="text-adm-muted">{j.route}</div>
                </td>
                <td className="whitespace-nowrap px-3 py-3 font-mono text-xs text-adm-ink-2">
                  {j.flightOut ?? "—"}
                  {j.flightReturn && <div className="text-adm-muted">D: {j.flightReturn}</div>}
                </td>
                <td className="max-w-[180px] px-3 py-3 text-xs text-adm-ink-2">{j.hotel ?? "—"}</td>
                <td className="whitespace-nowrap px-3 py-3 font-mono text-xs text-adm-ink-2">
                  {plates(j).length ? plates(j).map((p) => <div key={p}>{p}</div>) : "—"}
                </td>
                <td className="whitespace-nowrap px-3 py-3 text-end tabular-nums text-adm-ink">
                  {fmtMoney(j.fare.value, j.fare.currency)}
                  {j.payment === "cash" && <div className="text-[10px] font-semibold text-adm-muted">nakit</div>}
                </td>
                ...
                <td className="whitespace-nowrap px-3 py-3 text-end">
                  {j.tripType === "round_trip" ? <LegFee cell={j.ret} /> : <span className="text-adm-faint">—</span>}
                </td>
                <td className="whitespace-nowrap px-3 py-3 text-end tabular-nums text-adm-ink-2">
                  {j.cash ? (
                    <>
                      {fmtMoney(j.cash.value, j.cash.currency)}
                      {!j.cash.collectedByMe && <div className="text-[10px] text-adm-muted">diğer şoför</div>}
                    </>
                  ) : (
                ...
                <td className="whitespace-nowrap px-5 py-3 text-end tabular-nums text-adm-ink">
                  {j.driverNetUsd === null ? <span className="text-adm-faint">—</span> : fmtMoney(j.driverNetUsd, "USD")}
                </td>
              </tr>
            ))}
          </tbody>
          <tfoot>
            <tr className="border-t-2 border-adm-line bg-adm-surface-2 font-bold text-adm-ink">
  ...
  return (
    <span
      className={`font-bold tabular-nums ${
        m.usd === null ? "text-adm-amber" : m.usd < 0 ? "text-adm-green" : "text-adm-ink"
      }`}
    >
...
function Original({ m }: { m: MovementRow }) {
  ...
  return (
    <>
      ...
      {m.rate !== null && <span className="text-adm-muted"> · {rateText(m.original.currency, m.rate)}</span>}
    </>
  );
}
...
function Movements({
  ...
  const DeleteButton = ({ m }: { m: MovementRow }) =>
    m.manual ? (
      <button
        ...
        className="rounded-adm-sm p-1.5 text-adm-faint hover:bg-adm-rose-soft hover:text-adm-rose disabled:opacity-40"
      >
        <Trash2 size={14} />
      </button>
    ) : null;
  ...
  return (
    <>
      <div className="flex items-center justify-between bg-adm-surface-2 px-5 py-2.5 text-xs text-adm-muted">
        <span>Devreden bakiye</span>
        <span className="font-semibold tabular-nums text-adm-ink">{fmtMoney(s.opening, "USD")}</span>
      </div>

      {s.movements.length === 0 ? (
        <p className="px-5 py-12 text-center text-sm text-adm-muted">Bu dönemde hareket yok.</p>
      ) : (
        <>
          {/* Phones */}
          <ul className="divide-y divide-adm-line-2 md:hidden">
            {s.movements.map((m) => (
              <li key={m.id} className={`flex items-start gap-3 px-4 py-3 ${m.upcoming ? "opacity-60" : ""}`}>
                <div className="min-w-0 flex-1">
                  <p className="flex flex-wrap items-center gap-2 text-sm text-adm-ink">
                ...
                <div className="text-end">
                  <Amount m={m} />
                  <p className="text-[11px] tabular-nums text-adm-muted">{fmtMoney(m.balance, "USD")}</p>
                </div>
                <DeleteButton m={m} />
              </li>
            ))}
          ...
          <div className="hidden overflow-x-auto md:block">
            <table className="w-full min-w-[860px] text-sm">
              ...
              <tbody>
                {s.movements.map((m) => (
                  <tr key={m.id} className={`border-b border-adm-line-2 last:border-0 hover:bg-adm-surface-2/60 ${m.upcoming ? "text-adm-muted" : ""}`}>
                    <td className="whitespace-nowrap px-5 py-3 text-xs text-adm-muted">
                      {fmtDay(m.day)} <span className="text-adm-muted">{m.time}</span>
                    </td>
                    <td className="px-3 py-3">
                    ...
                    <td className="px-3 py-3 text-xs text-adm-ink-2">
                      {m.description}
                      {!m.manual && <span className="ms-1.5 text-[10px] text-adm-muted">otomatik</span>}
                      {m.upcoming && <span className="ms-1.5 text-[10px] text-adm-muted">ileri tarihli</span>}
                    </td>
                    <td className="whitespace-nowrap px-3 py-3 text-end text-xs tabular-nums text-adm-muted">
                      <Original m={m} />
                    </td>
                    <td className="whitespace-nowrap px-3 py-3 text-end">
                      <Amount m={m} />
                    </td>
                    <td className="whitespace-nowrap px-3 py-3 text-end tabular-nums text-adm-ink-2">{fmtMoney(m.balance, "USD")}</td>
      ...
      <div className="flex items-center justify-between border-t-2 border-adm-line bg-adm-surface-2 px-5 py-3 text-sm font-bold text-adm-ink">
